...
//    private final String password = "wxzt";                     //设置连接数据库的密码
    ...
    public Jdbc001() throws IOException {
    ...
    public int executeUpdate(String sql) throws IOException {
        ...
        try {
            stmt = con.createStatement();
            //根据数据库连接对象内容，创建执行SQL语句的声明对象
            int count = stmt.executeUpdate(sql);          //执行数据库更新的SQL语句
            System.out.println(count);
            return count;                                  //如果执行成功，则通过关键字return返回true
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            return -1;                                //如果执行失败，则通过关键字return返回false
        }
    }
