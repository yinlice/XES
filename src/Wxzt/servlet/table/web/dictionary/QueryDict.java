 ...
 */
public class QueryDict extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String strSQL="";//查询条件
        String sql ;//查询语句
        String DataTable = "px_CRM_Dict";//数据表
        ...
        if(!conditions.equals("")){
            ...
            strSQL += "and (DictType like '%"+conditions+"%' or DictMean like '%"+conditions+"%')";
        }
        /*字典所在页面*/
        String dictType = request.getParameter("dictType");
        if(Objects.equals(dictType, "1")){/*客户页面*/
            strSQL += " and DictType like 'px_CRM_Cust_%'";
        }else if(Objects.equals(dictType, "2")){/*工单页面*/
            strSQL += " and DictType like 'px_CRM_GD_%'";
        }else if(Objects.equals(dictType, "3")){/*其他页面*/
            strSQL += " and DictType not like 'px_CRM_Cust_%' and DictType not like 'px_CRM_GD_%'";
        }
        ...
        sql = "SELECT  * FROM " +DataTable+
                " WHERE 1=1 "+strSQL+" ORDER BY ID,DictType DESC limit "+currentCount+","+currentNum;
            ...
            try {
                ...
                try {
                    ...
                    while (os.next()) {
                        ...
                        data.add(os.getString("ID"));//ID0
                        data.add(os.getString("DictType"));//字典编号
                        data.add(os.getString("DictName"));//字典类别
                        data.add(os.getString("DictCode"));//字典value
                        data.add(os.getString("DictMean"));//字典显示值
                        data.add(os.getString("Note"));//备注
                        listData.add(data);
                    }
