...
public class custSql extends Query{
    ...
    public String setStrSqlType(HttpServletRequest request,String StrSqlOther){
        ...
        if(tableFormType==null|| Objects.equals(tableFormType, "")|| Objects.equals(tableFormType, "px_CRM_Cust_doctor")){
    ...
    public String[] getCustList(){
        ...
        try {
            otherList = get.getHeadList("DoctorHead","text");
        } catch (IOException e) {
            otherList = new String[]{"custid","custname","idnum","text01","text02","job","text04","text06","text07","text09","mobile","qq","weixin","email","deInvest","text10","text11","select03","text13","text14","addr"
            ,"text17","custFrom","text15","text16"};
            e.printStackTrace();
        }
        return otherList;
    }
    ...
    public String[] getCustSelectList(){
        ...
        try {
            selectList = get.getHeadList("DoctorHead","select");
        }catch (IOException e) {
            selectList = new String[]{
                    "select01","select02","select03","select04","custType","select05","custFrom","custNature"
            };
            e.printStackTrace();
        }
        return selectList;
    ...
    public String getCustSelectSql(String sql){
        setSqlSelect_T("a", getCustSelectList(), getCustList(), getCustWorkList(), getCustGroupList(), "px_CRM_Cust_doctor");
        /*设置含有下拉框信息的查询语句*/
    ...
    public String getCustSelectSql(String sql,HttpServletRequest request){
        ...
        setSqlSelect_T("a",ajaxOrJavaStringList(selectListS, getCustSelectList()),
                ajaxOrJavaStringList(textListS, getCustList()),getCustWorkList(),getCustGroupList(),"px_CRM_Cust_doctor");/*设置含有下拉框信息的查询语句*/
        return "select concat(a.custDynamicInfoNum ,'.['+DATE_FORMAT(a.dtUpdate,'%Y-%m-%d'),']<br>',a.custDynamicInfo )as custDynamicInfo, e.picName,"+sqlHeader+" from ("+sql+") as a "+sqlJoin +" "+
                "left join dddWork as d on a.worknumOwner = d.worknum " +
                "left join dddPic as e on d.picId = e.picId " ;
    }
    ...
    public String getCustSelectSqlNoPic(String sql,HttpServletRequest request){
        ...
        String textListS = request.getParameter("textList");
        setSqlSelect_T("a",ajaxOrJavaStringList(selectListS, getCustSelectList()),
                ajaxOrJavaStringList(textListS, getCustList()),getCustWorkList(),getCustGroupList(),"px_CRM_Cust_doctor");
        /*设置含有下拉框信息的查询语句*/
...
//        list.add("groupid");
    ...
    public boolean custRecycl(HttpServletRequest request){
        ...
        String sql = "update px_CRM_Cust_doctor set dtGive='"+newTime+"',dtUpdate='"+newTime+"',WorknumOwner='0' where Custid="+id;
    ...
    public boolean custDel(HttpServletRequest request){
        String id = request.getParameter("id");
        /*删除动态数据*/
        String sql = "delete from px_CRM_CustDynamic where custnum in (select custnum from px_CRM_Cust_doctor where Custid="+id+")";
        System.out.println("删除动态数据："+sql);
        boolean b = updataSql(sql);
        String sql1 = "delete from px_CRM_Cust_doctor where Custid="+id;
...
//        String worknumOwner = request.getParameter("worknumOwner");
        ...
        String sql = "update px_CRM_Cust_doctor "+setOneUpdate(request,setList)+",dtUpdate='"+newTime+"' where "+str;
        String action = request.getParameter("action");
        if(Objects.equals(action, "3")){/*添加动态时，更新联系时间*/
            sql = "update px_CRM_Cust_doctor "+setOneUpdate(request,setList)+",dtUpdate='"+newTime+"',dtContact='"+newTime+"' where "+str;
        }
    ...
    public boolean insertCust(HttpServletRequest request){
        ...
        try {
            ...
            if(custObject.size()==0){//不存在新客户
                ...
                String sql = "INSERT  into px_CRM_Cust_doctor "+insertList;
    ...
    public boolean extractCust(HttpServletRequest request){
        ...
        String sql = "update px_CRM_Cust_doctor set worknumOwner='"+workNum+"' ,dtGive='"+newTime+"',dtUpdate='"+newTime+"' " +
                "where custid in (select ab.custid from (select custid from px_CRM_Cust_doctor " +
    ...
    public boolean focusCust(HttpServletRequest request){
        ...
        String custid = request.getParameter("custid");
        String sql = "update px_CRM_Cust_doctor set isFocus="+isFocus+" ,dtGive='"+newTime+"',dtUpdate='"+newTime+"' " +
                "where custid="+custid;
        return updataSql(sql);
    }
