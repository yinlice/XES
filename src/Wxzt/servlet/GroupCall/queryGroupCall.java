 ...
 */
public class queryGroupCall extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        System.out.println("------------------queryGroupCall");
    ...
    /*action:0为查询页面，1为查询单条数据*/
    public Object getCustSQL(HttpServletRequest request,String action) {
        groupCallSql sql = new groupCallSql();
        if (Objects.equals(action, "0")) {
            return sql.groupCallQuery(request);
        }else if (Objects.equals(action, "1")) {
            return sql.groupCallOneQuery(request);
        }
        return null;
    }
}
